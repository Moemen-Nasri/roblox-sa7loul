...
-- WITH SAHLOUL AUTH, SPAWNER, FLY SPEED SLIDER, NOCLIP, MIC BYPASS
...
local function ToggleMicBypass()
    settings.MicBypass = not settings.MicBypass
    
    local audioIn = lp:FindFirstChild("AudioDeviceInput")
    if audioIn then
        audioIn.Muted = not settings.MicBypass
        notif("Mic Bypass: " .. (settings.MicBypass and "Enabled" or "Disabled"), 2)
    else
        notif("AudioDeviceInput not found", 2)
        settings.MicBypass = false
    end
end
...
-- ============================================
-- CESAR & SECHOIR SPAWNER
-- ============================================

local function IsCesarOrSechoir(obj)
    local name = string.lower(obj.Name)
    return (obj:IsA("Tool") or obj:IsA("Model") or obj:IsA("BasePart"))
        and (name:find("cesar", 1, true) ~= nil
            or name:find("césar", 1, true) ~= nil
            or name:find("sechoir", 1, true) ~= nil
            or name:find("shear", 1, true) ~= nil
            or name:find("scissor", 1, true) ~= nil)
end

local function GetAllCesarSechoirNames()
    local seen = {}
    local results = {}
    local function scan(root)
        if not root then return end
        for _, obj in ipairs(root:GetDescendants()) do
            if IsCesarOrSechoir(obj) and not seen[obj.Name] then
                seen[obj.Name] = true
...
local function FindCesarSechoir(itemName)
    ...
    if obj and IsCesarOrSechoir(obj) then return obj end
    return nil
end

local function SpawnCesarSechoir(itemName, targetPlayer)
    local target = targetPlayer or lp
    if not target then
        notif("No target player", 2)
        return false
    end
    local source = FindCesarSechoir(itemName)
    if not source then
...
local function SpawnAllCesarSechoir()
    local names = GetAllCesarSechoirNames()
    if #names == 0 then
        notif("No Cesar/Sechoir found in game", 2)
        return 0
    end
    local spawned = 0
    for _, itemName in ipairs(names) do
        if SpawnCesarSechoir(itemName, lp) then
            spawned = spawned + 1
        end
    end
...
-- MIC BYPASS SECTION
CreateSectionLabel(Content, "MIC BYPASS")
CreateToggle(Content, "Mic Bypass", settings.MicBypass, function(state)
    settings.MicBypass = state
    ToggleMicBypass()
end)

CreateButton(Content, "Unmute Mic", function()
    UnmuteMic()
end)

-- CESAR & SECHOIR SPAWNER SECTION
CreateSectionLabel(Content, "CESAR & SECHOIR SPAWNER")
CreateButton(Content, "Spawn All Cesar/Sechoir", function()
    SpawnAllCesarSechoir()
end)
